...
class STM_WP_Widget_Text extends WP_Widget {
	...
	public function __construct() {
		$widget_ops  = array(
			...
			'description' => __( 'STM Arbitrary text or HTML.', 'motors' )
		);
	...
	public function form( $instance ) {
		...
		<p><label
				for="<?php echo $this->get_field_id( 'text' ); ?>"><?php esc_html_e( 'Content:', 'motors' ); ?></label>
		...
		<p><input id="<?php echo $this->get_field_id( 'filter' ); ?>"
				...
				for="<?php echo $this->get_field_id( 'filter' ); ?>"><?php esc_html_e( 'Automatically add paragraphs', 'motors' ); ?></label>
		</p>
		<p><input id="<?php echo $this->get_field_id( 'socials' ); ?>"
		          name="<?php echo $this->get_field_name( 'socials' ); ?>"
		          type="checkbox" <?php checked( isset( $instance['socials'] ) ? $instance['socials'] : 0 ); ?> />&nbsp;<label
				for="<?php echo $this->get_field_id( 'socials' ); ?>"><?php esc_html_e( 'Add Socials Widget', 'motors' ); ?></label>
		</p>
	<?php
	}
}
