      ...
      <?php foreach($ticket_threads as $threadKey => $thread){ ?>
        ...
        <?php if($thread['sender_type']=='customer'){ ?>
            ...
            <div class="message-margin-manage">
              ...
              <div>
                ...
                <?php if($thread['receivers']){ ?>
                  ...
                  <?php echo (isset($thread['receivers']['bcc']) AND $thread['receivers']['bcc']) ? $text_bcc.': '.implode(',',$thread['receivers']['bcc']) : false; ?>
                <?php } ?>
              </div>
              <i>
            ...
            <div class="message">
              <div class="ticket-thread-actions hide">
                <?php if(in_array('tickets.split',$ts_roles)){ ?>
                  <button class="btn btn-sm btn-default" id="split-<?php echo $thread['id']; ?>" data-action="split" data-toggle="tooltip" title="<?php echo $text_split_ticket ;?>"><i class="fa fa-chain-broken"></i></button>
                <?php } ?>
              </div>
        ...
        <?php }elseif($thread['sender_type']=='agent'){ ?>
          ...
          <?php $agentNamePrint = str_replace('"', '', $agentNamePrint); ?>
            <?php if($current_agent['image'] AND $current_agent['id']==$thread['sender_id']) { ?>
            ...
            <div class="message-margin-manage">
              ...
              <i>
                ...
                <?php }elseif($thread['type']=='forward'){ ?>
                  <?php echo sprintf($text_agent_forwarded, $agentNamePrint, $thread['date_added']); ?>
                <?php }elseif($thread['type']=='note'){ ?>
                  <?php echo sprintf($text_agent_added_note, $agentNamePrint, $thread['date_added']); ?>
                <?php } ?>
              </i>
            </div>
            <div class="clearfix"></div>
            <div class="message<?php echo $thread['type']=='note' ? ' internal-div' : false; ?>">
              <div class="ticket-thread-actions hide">
                <?php if(in_array('tickets.deletethread',$ts_roles)){ ?>
                  <button class="btn btn-sm btn-default" id="deletethread-<?php echo $thread['id']; ?>" data-action="deletethread"><i class="fa fa-trash-o"></i></button>
                <?php } ?>
              </div>
              <?php if($thread['message'] != strip_tags(html_entity_decode($thread['message']))){ ?>
                <?php echo preg_replace('/<img/','<img class="img-responsive"', html_entity_decode($thread['message'], ENT_QUOTES, 'UTF-8')); ?>
