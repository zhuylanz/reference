	...
	public function index() {
		...
		if ($information_info) {
			...
			$data['description'] = html_entity_decode($information_info['description'], ENT_QUOTES, 'UTF-8');
		     	 ...
		     	 if (preg_match('/\[xformdata(.*?)\](.*?)\[\/xformdata\]/', $data['description'],$xforms)) {
					$xattrs = $xforms[1];
					$xtpl = $xforms[2];
					$xdata = array();
					if ($xattrs) {
					  $xattrs = trim($xattrs);
					  $xattrs = explode(' ',$xattrs);
					  foreach($xattrs as $xattr) {
					     list($key,$val) = explode('=',trim($xattr));
					     if ($key && $val) {
					       $xdata[$key] = $val;
					     }
					  }
					}
		        	$form_data=$this->model_xform_xform->getRenderData($xdata,$xtpl);
		        	$data['description'] = preg_replace('/\[xformdata(.*?)\](.*?)\[\/xformdata\]/', html_entity_decode($form_data), $data['description']);
		     	 }   
