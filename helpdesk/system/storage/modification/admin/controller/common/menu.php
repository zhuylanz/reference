        ...
        foreach ($ticketSystemLinks as $key => $value) {
            if(is_array($value)){
                foreach($value as $value2){
                    ...
                    if(!$tsBase->getAccessPermissionErrorStatus()){
                        $data['ticketSystem'][$key][] = array('href' => $this->url->link('ticketsystem/'.$value2, 'token=' . $this->session->data['token'], 'SSL'),
                ...
                // if($roleKey){
                    if(!$tsBase->getAccessPermissionErrorStatus())
                        $data['ticketSystem'][] = array('href' => $this->url->link('ticketsystem/'.$value, 'token=' . $this->session->data['token'], 'SSL'),
