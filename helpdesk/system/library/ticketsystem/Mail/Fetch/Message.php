...
class Message
{
    ...
    /**
     * This value defines the message no of mail
     *
     * @var integer
     */
    protected $messageNo;
   ...
   */
    ...
    protected function loadMessage()
    {
        ...
        $this->messageNo = $messageOverview->msgno;

        $this->subject = isset($messageOverview->subject) ? $messageOverview->subject : null;
        $this->subject = self::mimeHeaderDecode($this->subject);
   ...
   */
    ...
    public function getHeaders($forceReload = false)
    {
        if ($forceReload || !isset($this->headers)) {
            ...
            // var_dump($headerObject);
            //used imap_headerinfo function
            // $headerObject = imap_headerinfo($this->imapStream, $this->messageNo);
    ...
    public function getMessageBody($html = false)
    {
        if ($html) {
            if (!isset($this->htmlMessage) && isset($this->plaintextMessage)) {
                ...
                /**
                 *Webkul Replace cid message code from uploaded attchements
                 */
                if($this->getAttachments())
                    foreach($this->getAttachments() as $attachment) {
                        if(isset($attachment->getStructure()->id)){
                            // preg_replace("/$id/", $this->uid.'/'.$attachment->getRandonFileName(), $this->htmlMessage);
                            $this->htmlMessage = str_replace("cid:".preg_replace('/[<>]/','',$attachment->getStructure()->id) , HTTP_SERVER.'../image/'.$attachment->getSavePath().$attachment->getFileName(), $this->htmlMessage);
                        }
                    }
    ...
    {   
        ...
        if ((isset($parameters['name']) || isset($parameters['filename']))
            ...
            || ($structure->type >= 3)) //webkul
        ) {
            $attachment          = new Attachment($this, $structure, $partIdentifier);
            $this->attachments[] = $attachment;
            // $messageBody = imap_bodystruct($this->imapStream, $this->messageNo, $partIdentifier);
            ...
            if (!empty($parameters['charset']) && $parameters['charset'] !== self::$charset) {
                ...
                if (function_exists('mb_convert_encoding')) {
                    try {
                        /* Mail showing wrong encading so this function is throwing error */
                        $messageBody = mb_convert_encoding($messageBody, self::$charset, $parameters['charset']);
                        $mb_converted = true;
                    } catch (Exception $e) {
                        // $messageBody = mb_convert_encoding($messageBody, self::$charset, mb_detect_encoding($messageBody));
                        // var_dump($e);
        ...
        if (is_array($addresses))
            foreach ($addresses as $address) {
                ...
                if (isset($address->personal))
                    $currentAddress['name'] = self::mimeHeaderDecode($address->personal);
