...
class Attachment
{
    ...
    /**
     * Webkul - This will used when images / videos are in message body (without path) then it shows their extension.
     *
     * @internal
     * @var string
     */
    protected $attachmentExt;

    /**
     * Webkul - This will used when images / videos are in message body (without path) then it shows their random name.
     *
     * @internal
     * @var string
     */
    ...
    /**
     * Webkul - This will used when images / videos save path.
     *
     * @internal
     * @var string
     */
    ...
    public function __construct(Message $message, $structure, $partIdentifier = null)
    {
        ...
        $this->attachmentExt = strtolower($structure->subtype);
        ...
        $this->size = property_exists($structure, 'bytes') ? $structure->bytes : 0;
        ...
        $path = 'helpdesk/email/'.$this->messageId.'/';

        /*Save attached files */
        $this->saveToDirectory($path);
    }
    ...
    {
        return (isset($this->filename)) ? $this->filename : self::getRandonFileName();
    }

    /**
     * This returns the random filename of the attachment, when filename is not exits
     *
     * @return string
     */
    public function getRandonFileName()
    {   
        if($this->randomFileName)
            return $this->randomFileName;

        $this->randomFileName = rand().'.'.$this->attachmentExt;
        return $this->randomFileName;
    }
    ...
    /**
     * This returns the saved path of the attachment.
     *
     * @return string
     */
    ...
    {   
        ...
        $path = $this->savePath = rtrim($path, '/') . '/';

        if (is_dir(DIR_IMAGE.$path))
            return $this->saveAs(DIR_IMAGE.$path . $this->getFileName());
